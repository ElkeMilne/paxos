...
public class Member {
    ...
    private final String proposalVal; // Default proposal value (set to memberId).
    ...
    public void setHighestSeenProposalNumber(String proposalNum) {
        this.latestProposalId = proposalNum;
    }
    ...
    public void sendPrepareRequest() {
        ...
        String proposalNum = generateProposalNumber();
    ...
    public void sendPromise(String proposerId, String proposalNum, AcceptedProposal acceptedProposalPair) {
        if (goOffline()) {
            return; // Skip if offline.
        ...
        if (acceptedProposalPair == null) {
            communicate.sendMessage(proposerId, "PROMISE " + proposalNum);
        } else {
            communicate.sendMessage(proposerId, "PROMISE " + proposalNum + " "
    ...
    public synchronized void sendAcceptRequest(String proposalNum, String proposalVal, String currentMemberId) {
        ...
        if (proposalVal == null) {
            proposalVal = currentMemberId; // Default value.

        }
        votingServer.broadcast("ACCEPT " + proposalNum + " " + proposalVal);
    }

    // Send an ACCEPTED message to a proposer.
    public void sendAccepted(String proposerId, String proposalNum, String comingAcceptedValue) {
        if (goOffline()) {
            return; // Skip if offline.
        ...
        communicate.sendMessage(proposerId, "ACCEPTED " + proposalNum + " " + comingAcceptedValue);
    }

    // Send a REJECT message.
    public void sendReject(String proposalNum) {
        if (goOffline()) {
            return; // Skip if offline.
        ...
        communicate.sendMessage(memberId, "REJECT " + proposalNum);
    }

    // Broadcast the RESULT of a vote.
    public void sendResult(String proposalNum, String proposalVal) {
        if (goOffline()) {
            return; // Skip if offline.
        ...
        votingServer.broadcast("RESULT " + proposalNum + " " + proposalVal);
    }
}
